...
def train():
    ...
    class_df.to_csv('ai/class.csv', index=False)

    class_json = {classses.get(class_name, class_name): 0 for class_name in label_encoder.classes_}
    with open('index/class_counts.json', 'w', encoding='utf-8') as json_file:
        json.dump(class_json, json_file, ensure_ascii=False)